...
@section('content')
@include('includes.page-header', ['title' => 'تعديل تلميذ'])

<div class="row">
    <div class="col-md-12 col-sm-12">
        <div class="card card-box">
            <div class="card-head">
                <header>تعديل تلميذ</header>
            </div>
            <div class="card-body " id="bar-parent">
                <form action="{{ route('students.update', ['student' => $student]) }}" method="post">
                    ...
                    <div class="form-group @error('nom') has-error @enderror">
                        ...
                        <input type="text" name="nom" value="{{ $student->nom }}"  class="form-control" id="simpleFormEmail" placeholder="إدخال الإسم">
                        @error('nom')
                    ...
                    <div class="form-group @error('prenom') has-error @enderror">
                        ...
                        <input type="text" name="prenom" value="{{ $student->prenom }}" class="form-control" id="simpleFormEmail" placeholder="إدخال اللقب">
                        @error('prenom')
                            <span class="help-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group @error('niveau_id') has-error @enderror">
                        ...
                        <select class="form-select" name="niveau_id">
                            <option>إختيار مرحلة</option>
                            @foreach(App\Models\Niveau::all() as $niveau)
                                <option value="{{ $niveau->id }}" @if($student->niveau_id == $niveau->id) selected @endif> {{ $niveau->label }}</option>
                            @endforeach
                        </select>
                        @error('niveau_id')
                            <span class="help-block">{{ $message }}</span>
                    ...
                    <div class="form-group @error('email') has-error @enderror">
                        ...
                        <input type="email" name="email" value="{{ $student->email }}" class="form-control" id="simpleFormEmail" placeholder="إدخال البريد الإلكتروني">
                        @error('email')
                            <span class="help-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group       @error('date_naissance') has-error @enderror"> 
                        ...
                        <input type="date" name="date_naissance" value="{{ $student->date_naissance }}" class="form-control" id="simpleFormEmail">
                    ...
                    <div class="form-group @error('genre') has-error @enderror">
                        ...
                        <select class="form-select" name="genre">
                           ...
                           <option value="male" @if($student->genre == "male") selected @endif>ذكر</option>
                           <option value="female" @if($student->genre == "female") selected @endif>أنثى</option>
                        </select>
                        @error('genre')
                            <span class="help-block">{{ $message }}</span>
                        @enderror
